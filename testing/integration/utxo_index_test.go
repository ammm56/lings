...
func TestUTXOIndex(t *testing.T) {
	...
	err := lings.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	getCoinSupplyResponse, err := lings.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error Retriving Coin supply: %s", err)
	}

	rewardsMinedSompi := uint64(blockAmountToMine * constants.SompiPerLings * 500)
	getBlockCountResponse, err := lings.rpcClient.GetBlockCount()
	if err != nil {
		t.Fatalf("Error Retriving BlockCount: %s", err)
	}
	rewardsMinedViaBlockCountSompi := uint64(
		(getBlockCountResponse.BlockCount - 2) * constants.SompiPerLings * 500, // -2 because of genesis and virtual.
	)
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = lings.rpcClient.SubmitTransaction(rpcTransaction, transactionID, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := lings.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
...
func buildTransactionForUTXOIndexTest(t *testing.T, entry *appmessage.UTXOsByAddressesEntry) (*appmessage.RPCTransaction, string) {
	...
	payeeAddress, err := util.DecodeAddress(miningAddress1, util.Bech32PrefixLingsSim)
	if err != nil {
