...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of lings instances
	lings1, lings2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := lings1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In lings2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, lings2)
	}

	// Connect the two lingss. This should trigger sync
	// between the two nodes
	connect(t, lings1, lings2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := lings1.rpcClient.GetVirtualSelectedParentChainFromBlock(
