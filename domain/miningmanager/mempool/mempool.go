...
import (
	...
	miningmanagermodel "github.com/ammm56/lings/domain/miningmanager/model"
)
...
func (mp *mempool) BlockCandidateTransactions() []*externalapi.DomainTransaction {
	...
	for _, tx := range readyTxs {
		if len(tx.Outputs) > len(tx.Inputs) {
			...
			if !hasCoinbaseInput && numExtraOuts > 2 && tx.Fee < uint64(numExtraOuts)*constants.SompiPerLings {
				log.Debugf("Filtered spam tx %s", consensushashing.TransactionID(tx))
				continue
			}

			if hasCoinbaseInput || tx.Fee > uint64(numExtraOuts)*constants.SompiPerLings {
