...
func sign(conf *signConfig) error {
	...
	for i, partiallySignedTransaction := range partiallySignedTransactions {
		updatedPartiallySignedTransactions[i], err =
			liblingswallet.Sign(conf.NetParams(), privateKeys, partiallySignedTransaction, keysFile.ECDSA)
		if err != nil {
			return err
		}
	}
	...
	for _, updatedPartiallySignedTransaction := range updatedPartiallySignedTransactions {
		...
		isFullySigned, err := liblingswallet.IsTransactionFullySigned(updatedPartiallySignedTransaction)
		if err != nil {
			return err
		}
		if !isFullySigned {
