...
import (
	...
	"github.com/ammm56/lings/cmd/lingswallet/liblingswallet/serialization"
...
func (s *server) mergeTransaction(
	...
	utxos := make([]*liblingswallet.UTXO, len(splitTransactions))
	for i, splitTransaction := range splitTransactions {
		output := splitTransaction.Tx.Outputs[0]
		utxos[i] = &liblingswallet.UTXO{
	...
	payments := []*liblingswallet.Payment{{
		Address: toAddress,
		Amount:  sentValue,
	}}
	if totalValue > sentValue {
		payments = append(payments, &liblingswallet.Payment{
			Address: changeAddress,
			Amount:  totalValue - sentValue,
		})
	}

	mergeTransactionBytes, err := liblingswallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) splitAndInputPerSplitCounts(transaction *serialization.PartiallySignedTransaction, transactionMass uint64,
	...
	// Since the transaction was generated by lingswallet, we assume all inputs have the same number of signatures, and
	// thus - the same mass.
...
func (s *server) createSplitTransaction(transaction *serialization.PartiallySignedTransaction,
	...
	selectedUTXOs := make([]*liblingswallet.UTXO, 0, endIndex-startIndex)
	totalSompi := uint64(0)
	...
	for i := startIndex; i < endIndex && i < len(transaction.PartiallySignedInputs); i++ {
		...
		selectedUTXOs = append(selectedUTXOs, &liblingswallet.UTXO{
	...
	unsignedTransactionBytes, err := liblingswallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) estimateMassAfterSignatures(transaction *serialization.PartiallySignedTransaction) (uint64, error) {
	...
	transactionWithSignatures, err := liblingswallet.ExtractTransactionDeserialized(transaction, s.keysFile.ECDSA)
	if err != nil {
		return 0, err
	}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*liblingswallet.UTXO, requiredAmount uint64) (
	additionalUTXOs []*liblingswallet.UTXO, totalValueAdded uint64, err error) {
	...
	for _, utxo := range s.utxosSortedByAmount {
		...
		additionalUTXOs = append(additionalUTXOs, &liblingswallet.UTXO{
