...
import (
	...
	"github.com/ammm56/lings/domain/consensus"
	...
	"github.com/ammm56/lings/domain/consensus/utils/testutils"
...
func TestMultisig(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			for i := 0; i < numKeys; i++ {
				var err error
				mnemonics[i], err = liblingswallet.CreateMnemonic()
				if err != nil {
					t.Fatalf("CreateMnemonic: %+v", err)
				}

				publicKeys[i], err = liblingswallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], true)
				if err != nil {
			...
			path := "m/1/2/3"
			address, err := liblingswallet.Address(params, publicKeys, minimumSignatures, path, ecdsa)
			if err != nil {
			...
			selectedUTXOs := []*liblingswallet.UTXO{
				{
			...
			unsignedTransaction, err := liblingswallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
			...
			isFullySigned, err := liblingswallet.IsTransactionFullySigned(unsignedTransaction)
			if err != nil {
			...
			_, err = liblingswallet.ExtractTransaction(unsignedTransaction, ecdsa)
			...
			signedTxStep1, err := liblingswallet.Sign(params, mnemonics[:1], unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			isFullySigned, err = liblingswallet.IsTransactionFullySigned(signedTxStep1)
			if err != nil {
				t.Fatalf("IsTransactionFullySigned: %+v", err)
			}
			...
			signedTxStep2, err := liblingswallet.Sign(params, mnemonics[1:2], signedTxStep1, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			extractedSignedTxStep2, err := liblingswallet.ExtractTransaction(signedTxStep2, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}

			signedTxOneStep, err := liblingswallet.Sign(params, mnemonics[:2], unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			extractedSignedTxOneStep, err := liblingswallet.ExtractTransaction(signedTxOneStep, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
	...
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
			...
			for i := 0; i < numKeys; i++ {
				...
				publicKeys[i], err = liblingswallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], false)
				if err != nil {
					t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
				}
			}
			...
			signedTx, err := liblingswallet.Sign(params, mnemonics, unsignedTransaction, ecdsa)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			tx, err := liblingswallet.ExtractTransaction(signedTx, ecdsa)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
...
func TestMaxSompi(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		for i := 0; i < numKeys; i++ {
			...
			publicKeys[i], err = liblingswallet.MasterPublicKeyFromMnemonic(&cfg.Params, mnemonics[i], false)
			if err != nil {
				t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
			}
		}
		...
		const minimumSignatures = 1
		path := "m/1/2/3"
		address, err := liblingswallet.Address(params, publicKeys, minimumSignatures, path, false)
		if err != nil {
			t.Fatalf("Address: %+v", err)
		}
		...
		selectedUTXOsForTxWithLargeInputAmount := []*liblingswallet.UTXO{
			{
				Outpoint: &externalapi.DomainOutpoint{
		...
		unsignedTxWithLargeInputAmount, err := liblingswallet.CreateUnsignedTransaction(publicKeys, minimumSignatures,
			[]*liblingswallet.Payment{{
				Address: address,
				Amount:  10,
		...
		signedTxWithLargeInputAmount, err := liblingswallet.Sign(params, mnemonics, unsignedTxWithLargeInputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		txWithLargeInputAmount, err := liblingswallet.ExtractTransaction(signedTxWithLargeInputAmount, false)
		if err != nil {
			t.Fatalf("ExtractTransaction: %+v", err)
		}
		...
		selectedUTXOsForTxWithLargeInputAndOutputAmount := []*liblingswallet.UTXO{
			{
				Outpoint: &externalapi.DomainOutpoint{
		...
		signedTxWithLargeInputAndOutputAmount, err := liblingswallet.Sign(params, mnemonics, unsignedTxWithLargeInputAndOutputAmount, false)
		if err != nil {
			t.Fatalf("Sign: %+v", err)
		}

		txWithLargeInputAndOutputAmount, err := liblingswallet.ExtractTransaction(signedTxWithLargeInputAndOutputAmount, false)
		if err != nil {
			t.Fatalf("ExtractTransaction: %+v", err)
		}
