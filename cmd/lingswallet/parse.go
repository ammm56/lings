...
func parse(conf *parseConfig) error {
	...
	for i, transaction := range transactions {
		...
		for index, input := range partiallySignedTransaction.Tx.Inputs {
			...
			if conf.Verbose {
				fmt.Printf("Input %d: \tOutpoint: %s:%d \tAmount: %.2f Lings\n", index, input.PreviousOutpoint.TransactionID,
					input.PreviousOutpoint.Index, float64(partiallySignedInput.PrevOutput.Value)/float64(constants.SompiPerLings))
			}
		...
		for index, output := range partiallySignedTransaction.Tx.Outputs {
			...
			fmt.Printf("Output %d: \tRecipient: %s \tAmount: %.2f Lings\n",
				index, addressString, float64(output.Value)/float64(constants.SompiPerLings))
