...
// long-running lingsMain (which is the real meat of lings), handles service
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// Start lingsMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once lings is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("lingsMain-windows", func() {
...
// logServiceStart logs information about lings when the main server has
