...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-lings.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "lings")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of lings binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-lings.conf")
