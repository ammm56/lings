...
import (
	...
	"github.com/ammm56/lings/util/network"
...
const (
	defaultConfigFilename      = "lings.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "lings.log"
	defaultErrLogFilename      = "lings_err.log"
	...
	sampleConfigFilename    = "sample-lings.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for lings.
	DefaultAppDir = util.AppDir("lings", false)
...
// Flags defines the configuration options for lings.
//
...
type Flags struct {
	...
	MinRelayTxFee                   float64       `long:"minrelaytxfee" description:"The minimum transaction fee in LSN/kB to be considered a non-zero fee."`
...
// Config defines the configuration options for lings.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default lings configuration
...
//
// The above results in lings functioning properly without any config settings
...
// createDefaultConfig copies the file sample-lings.conf to the given destination path,
