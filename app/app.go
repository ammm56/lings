...
import (
	...
	"github.com/ammm56/lings/infrastructure/os/execenv"
	"github.com/ammm56/lings/infrastructure/os/limits"
	"github.com/ammm56/lings/infrastructure/os/signal"
	"github.com/ammm56/lings/infrastructure/os/winservice"
...
type lingsApp struct {
	cfg *config.Config
}

// StartApp starts the lings app, and blocks until it finishes running
func StartApp() error {
...
func (app *lingsApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start lings: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down lings...")
		...
		log.Infof("lings shutdown complete")
	}()
