...
// Outpoint defines a lings data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new lings transaction outpoint point with the
...
// TxIn defines a lings transaction input.
type TxIn struct {
...
// NewTxIn returns a new lings transaction input with the provided
...
// TxOut defines a lings transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new lings transaction output with the provided
...
// MsgTx implements the Message interface and represents a lings tx message.
