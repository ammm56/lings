/*
Package appmessage implements the lings appmessage protocol.

At a high level, this package provides support for marshalling and unmarshalling
supported lings messages to and from the appmessage. This package does not deal
...
which lings network it is a part of, its type, how big it is, and a checksum
...
of the currently supported lings messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using lings encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the lings messages are intended
...
and write lings messages. The following sections provide a quick overview of
...
As discussed in the lings message overview section, this package reads
and writes lings messages using a generic interface named Message. In
...
In order to unmarshall lings messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a lings peer. Example syntax is:
	...
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, lingsNet)
	if err != nil {
...
In order to marshall lings messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a lings peer. Example syntax to request addresses
from a remote peer is:
	...
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, lingsNet)
	if err != nil {
		// Log and handle the error
	}
