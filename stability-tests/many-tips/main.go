...
import (
	...
	"github.com/ammm56/lings/domain/consensus/utils/mining"
	"github.com/ammm56/lings/util"
	"github.com/kaspanet/go-secp256k1"

	"github.com/ammm56/lings/stability-tests/common"
	"github.com/ammm56/lings/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("lings-datadir")
	if err != nil {
	...
	spawn("startNode-lingsCmd.Wait", func() {
		err := lingsCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("lingsCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes lings closes ungracefully
				log.Errorf("lingsCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(lingsCmd, "lingsCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	lingsMinerCmd, err := common.StartCmd("MINER",
		"lingsminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("lings-miner-Cmd.Wait", func() {
		err := lingsMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(lingsMinerCmd, "lingsMinerCmd")
	return nil
}
